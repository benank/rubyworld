export class Player {
    ...
    render(ctx: CanvasRenderingContext2D, tileSize: number) {
      const drawX = this.isMoving
        ? (this.x * (1 - this.movementProgress) + this.targetX * this.movementProgress) * tileSize
        : this.x * tileSize
      const drawY = this.isMoving
        ? (this.y * (1 - this.movementProgress) + this.targetY * this.movementProgress) * tileSize
        : this.y * tileSize
    ...
    getPosition() {
      const x = this.isMoving ? this.x * (1 - this.movementProgress) + this.targetX * this.movementProgress : this.x
      const y = this.isMoving ? this.y * (1 - this.movementProgress) + this.targetY * this.movementProgress : this.y
      return { x, y }
    }
  }
